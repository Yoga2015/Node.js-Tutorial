...
const server = http.createServer((req, res) => {      // 请求处理 回调函数 ：当客户端发送请求时，服务器会调用这个回调函数，处理请求并返回响应
    ...
    // 检查请求头中是否包含X-Magic-Key
    const magicKey = req.headers['x-magic-key'];

    if (magicKey === 'OpenSesame') {
        res.writeHead(200, {'Content-Type': 'text/plain'});
        res.end('暗号正确！欢迎进入秘密基地 🚪');
    } else {
...
//  想启动 这个简单的服务器 ， 打开终端（控制台） ， 输入 node server.js 回车 就可以了，

//  控制台里会 打印出 这条消息 “服务器运行在 http://localhost:3000/”，说明服务器已经启动了
